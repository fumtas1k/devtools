...
test.describe('JSON / CSV 変換', () => {
  ...
  test('JSON → CSV: サンプルデータを変換できる', async ({ page }) => {
    ...
    await expect(page.locator('#json-csv-output')).not.toHaveValue('');
    await expect(page.locator('#json-csv-output')).toHaveValue(/id/);
    await expect(page.locator('#json-csv-output')).toHaveValue(/name/);
  });

  test('JSON → CSV: フラットなJSONをCSVに変換する', async ({ page }) => {
    await page.getByLabel('入力').fill('[{"id":1,"name":"太郎"},{"id":2,"name":"花子"}]');
    await expect(page.locator('#json-csv-output')).toHaveValue(/id,name/);
    await expect(page.locator('#json-csv-output')).toHaveValue(/太郎/);
    await expect(page.locator('#json-csv-output')).toHaveValue(/花子/);
  });

  test('JSON → CSV: ネストオブジェクトをドット記法でフラット化する', async ({ page }) => {
    await page.getByLabel('入力').fill('[{"name":"太郎","address":{"city":"東京"}}]');
    await expect(page.locator('#json-csv-output')).toHaveValue(/address\.city/);
    await expect(page.locator('#json-csv-output')).toHaveValue(/東京/);
  });
  ...
  test('CSV → JSON: サンプルデータを変換できる', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'サンプルを入力' }).click();
    await expect(page.locator('#json-csv-output')).not.toHaveValue('');
    await expect(page.locator('#json-csv-output')).toHaveValue(/\[/);
  });
