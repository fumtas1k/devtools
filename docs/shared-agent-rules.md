...
| 変更の種類           | 更新が必要なファイル                                                                            |
| :------------------- | :---------------------------------------------------------------------------------------------- |
| ツール追加           | `README.md` (ツール一覧), `SPEC.md` (4, 5, 9章, 構成, ライブリ), `docs/decisions.md` (選定理由) |
| ツール削除・slug変更 | 上記すべて                                                                                      |
...
## 4. UI 変更時の目視確認 (Playwright)

UI コンポーネントやレイアウトを変更した場合、コミット前に Playwright を使用して **PCサイズ (1280x800)** と **スマホサイズ (390x844)** の両方でスクリーンショットを撮影し、目視で意図通りか確認すること。
