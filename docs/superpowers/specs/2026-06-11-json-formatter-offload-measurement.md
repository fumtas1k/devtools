...
TreeNode は関数・循環参照を含まない。Node AST は `parent` 双方向参照を持つ循環構造だが、`structuredClone` は循環参照に対応するため clone 自体は成功する（cloneable）。ただし **Node AST の clone コストが極めて高い**（後述・§5-3）。
