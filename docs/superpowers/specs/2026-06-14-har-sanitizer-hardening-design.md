...
> ルール別計測で O(n²) の**唯一の主因は `EMAIL` ルール** `[\w.+-]+@[\w-]+(?:\.[\w-]+)+` と判明（40k 文字・`@` 無し長語連で 1461ms、他ルールは全て <5ms）。`HIGH_ENTROPY` は単一の greedy マッチで O(n) のため無関係だった。
> 当初案の「`{24,}`/`{32,}` を `{24,512}` に上限化」は**有害**: 512字超の長いトークンで末尾 `\b` が見つからずバックトラックが多発し、**逆に新たな O(n²) を生む**（実機確認）。よって採用しない。

- **真因修正**: `rules.ts` の `EMAIL` ルールの量化子を RFC 準拠の上限付きにする: `[\w.+-]{1,64}@[\w-]{1,63}(?:\.[\w-]{1,63})+`（local part ≤64・label ≤63）。各開始位置のバックトラックが定数で打ち切られ O(n) になる。実機で全体が O(n²)→O(n)（80k: 5721ms→29ms）に改善、実在メールの検出は不変を確認済み。
  - 上限超過の「メール風」文字列は RFC 上も無効なため、実害のある検出損失は無い（既知の境界として記録）。
- **回帰防止**: 大入力（`@` 無し長語連）に対し `scrubText` が一定時間内（線形スケール）に完了する性能 assert を陽性対照として併設する。旧 EMAIL regex に当てるとテストがタイムアウト/閾値超過で fail する設計にする。
