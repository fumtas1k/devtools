...
> 入力は `InputField rows={18}`）へ変更した。
> さらにレビュー反映で、**ダウンロード＋コピーをプレビュー（結果）側ヘッダ**に配置し（変換系ツール標準）、
> コピー表記は「HTMLをコピー」→「HTMLコピー」に短縮、**下部アクション行は `ClearButton`**（入力リセット）とした。
> 最終的な確定形は設計書 §3〜§4 と実装コミットを正とする（本計画の以降のコードブロックは初版の記録）。
