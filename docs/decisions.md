...
## [コントラスト比マトリクス] APCA の自前実装採用・外部ライブラリ非採用

**2026-06-25 | ステータス: 採用**

### 背景

コントラスト比マトリクスツール（`contrast-matrix`）で APCA-W3 0.1.9 のコントラスト指標を実装するにあたり、公式 npm パッケージ（`@color-contrast/apca-w3` 等）の採用 vs 自前実装を検討した。

### 自前実装を採用した理由

- **W3C ベータライセンスの不確実性**: APCA-W3 は現時点でベータ仕様であり、W3C のライセンス条件が明確でない。本プロジェクトはオープンソース公開を前提としており、ライセンス非適合のリスクを避けるため自前実装を選択。
- **依存追加ゼロ**: `contrast.ts` は純粋計算のみで構成され、外部依存なしで `parseColor` / `relativeLuminance` / `contrastRatio` / `wcagLevels` / `apcaLc` / `buildMatrix` を実装できた。
- **アルゴリズムの透明性**: APCA-W3 0.1.9 の公式仕様（定数・式）をコメント付きで実装することで、将来の仕様変更（定数更新等）への追従がコード内で完結する。
- **実装の容易さ**: APCA-W3 0.1.9 のアルゴリズムは定数と数式で完全に記述されており、50 行程度で正確に実装できる（WCAG 21:1 / APCA 106.04 等の参照値で Vitest で検証済み）。

### トレードオフ

- ✅ 依存追加なし・バンドルサイズへの影響ゼロ
- ✅ APCA 定数・式をコードで明示的に管理（ブラックボックスにならない）
- ⚠️ APCA がベータ版を脱して仕様が変更された場合、`src/utils/contrast.ts` を手動更新する必要がある
- ⚠️ v1 は不透明色のみ対応（アルファ合成は実装しない）

---

## [日本語ダミー個人データ生成] faker 不採用・自前辞書採用

**2026-06-27 | ステータス: 採用**

### 背景

日本語ダミー個人データ生成ツール（`dummy-personal-data`）の実装にあたり、氏名・住所等の日本語データ生成手段として `faker` の ja ロケール vs 自前辞書を検討した。

### 自前辞書を採用した理由

- **バンドルサイズ**: `faker` は全ロケールで数 MB 規模。ja ロケール限定でも本プロジェクトの既存バンドルへの増分が許容できない（`DummyText` と同様に `Math.random()` + 自前辞書方式を踏襲する）。
- **整合性制御**: 氏名↔フリガナ・住所↔郵便番号↔市外局番・生年月日↔年齢の整合ロジックを自前で制御できる。`faker` は個々フィールドを独立生成するため整合検証が別途必要になる。
- **依存追加ゼロ**: 新規 npm パッケージ追加なし（`papaparse` は既存を再利用）。

---

## [日本語ダミー個人データ生成] 携帯番号帯域 0900/0700（C=0）採用方針

**2026-06-27 | ステータス: 採用**

### 背景

ダミー携帯番号が実在の番号に当たらないよう、生成帯域の選定基準を定める必要があった。

### 決定: 090/070 + 第 4 桁 C=0 を採用、0600/0800 は除外

- **根拠**: 総務省「電話番号に関する情報」の「電気通信番号の種別」表では、音声伝送携帯電話番号の構成を `0X0CDEFGHJK`（060/070/080/090）と定め、**C は 0 を除く** と規定している。よって第 4 桁 C=0 の帯域（`0900XXXXXXX` / `0700XXXXXXX`）は音声携帯の割当対象外であり、実在しない確度が極めて高い。
- **0800 除外**: フリーダイヤル番号として割当済みのため生成対象から除外。
- **0600 除外**: 同表に FMC 電話番号 `0600DEFGHJK` として割当済みの番号種別が存在するため除外。
- **C=0 のみ対象**: `0901`（C=1）以上は実在音声携帯帯のため対象外。

### トレードオフ

- ✅ 実在の音声携帯・フリーダイヤル・FMC いずれとも重複しない帯域を選択できる
- ✅ 番号計画上の明確な根拠あり（総務省公示の割当規則）
- ⚠️ 制度変更で C=0 帯域に新種別が割当られた場合は方針を見直す必要がある

---

## [日本語ダミー個人データ生成] Excel 非対応・CSV BOM 付きで Excel 互換確保

**2026-06-27 | ステータス: 採用**

### 背景

ダウンロード形式として CSV / JSON を採用し、Excel（.xlsx）形式は v1 対象外とした。

### 決定理由

- **バンドルサイズ**: `xlsx`（SheetJS）/ `exceljs` は数百 KB 〜 数 MB のバンドル増分が生じる。日本語テストデータ用途には過剰。
- **CSV の Excel 互換**: UTF-8 BOM（U+FEFF）を CSV 先頭に付加することで、Excel が自動的に UTF-8 と認識し文字化けを防ぐ。日本語テストデータの主要な開いてほしいエディタ（Excel / LibreOffice / Numbers）で動作確認。
- **既存 `papaparse` 再利用**: 新規ライブラリ追加なし。

### トレードオフ

- ✅ 依存追加ゼロ・バンドルサイズへの影響ゼロ
- ✅ CSV + BOM で Excel の日本語表示は実用上問題なし
- ⚠️ テーブル装飾・数式・複数シートは不可。純粋なデータ出力のみ
