...
- **カンマ位置**: 行末（既定）/ 先頭を切り替えられる。`sql-formatter` v15 で `commaPosition` オプションが廃止された（指定すると例外）ため、先頭カンマは整形結果に対する後処理で実現する。行末（`,`）にあるカンマだけを次行の先頭へ移動し（インデントは保持）、文字列・コメント内のカンマ（行末に来ない）は変換しない。
