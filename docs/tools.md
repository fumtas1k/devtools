...
- **重複検出**: 各 CIDR をネットワークアドレスと末尾アドレス（`start + totalCount - 1`）に変換し、全ペアを `O(n²)` で比較。`[aStart,aEnd]` と `[bStart,bEnd]` の包含・一致・交差を BigInt 比較で判定。バージョン混在ペアはスキップ。解析失敗行は行単位でエラー収集し、有効行の重複判定は継続する。
