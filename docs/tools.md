...
- **氏名↔フリガナ整合**: 姓・名それぞれの辞書（`{ kanji, yomi, romaji }`）から 1 件ずつランダム選択し、漢字と読みが同一辞書エントリの組み合わせになることを保証する。辞書には姓 30 件・男名 20 件・女名 20 件を収録（`src/utils/dummy-personal-data/dictionaries.ts`）。
