...
export interface Dialect {
  ...
  srv: boolean;
  /**
   * JDBC URL 形式（`jdbc:` プレフィックス＋ credential を userinfo でなく
   * `?user=&password=` プロパティに置く）。PostgreSQL / MySQL のみ対応。
   */
  jdbc: boolean;
...
export const DIALECTS: Record<DsnScheme, Dialect> = {
  ...
  'jdbc:mysql': {
    ...
    srv: false,
    jdbc: true,
    sample: 'jdbc:mysql://db.example.com:3306/app_db?user=app_user&password=p%40ssw0rd&useSSL=true',
  },
};
