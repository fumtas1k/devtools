...
- **PR 作成**: 4 点必須（正本: `docs/playbooks/pr-creation.md`、不一致時は playbook 優先）:
  ...
  3. pre-create check: 以下 3 つを全て満たす
     - develop ベース一致: `git rev-parse origin/develop` == `git merge-base HEAD origin/develop`
     - スコープ確認: `git diff origin/develop --name-only` で想定外ファイルなし
     - a11y 保護: aria-\* 削除行なし（支援技術が依存する semantic マーカーの意図せぬ消失検知。詳細は `docs/shared-agent-rules.md` 9.6 章）
  4. 言語: タイトル・本文 必ず日本語
